
import { ProjectData } from './types';

export const projects: ProjectData[] = [
  {
    id: 'soulware-ecosystem',
    title: 'The Soulware™ Ecosystem',
    category: 'Fintech / Social Impact',
    fundingAsk: '$10.5M Seed',
    valuation: '$105M Pre-money',
    imageUrl: 'https://images.unsplash.com/photo-1582213782179-e0d53f98f2ca?auto=format&fit=crop&w=800&q=80',
    pitchDeckUrl: '', 
    tags: ['Fintech', 'Impact Investing', 'Financial Inclusion'],
    shortDescription: {
      en: "A comprehensive ecosystem delivering financial dignity to LGBTQ+, seniors, and migrants with measurable social impact.",
      es: "Un ecosistema integral que ofrece dignidad financiera a comunidades LGBTQ+, personas mayores y migrantes con impacto social medible.",
      fr: "Un écosystème complet offrant la dignité financière aux LGBTQ+, aux personnes âgées et aux migrants avec un impact social mesurable.",
      de: "Ein umfassendes Ökosystem, das LGBTQ+, Senioren und Migranten finanzielle Würde mit messbarer sozialer Wirkung bietet."
    },
    fullDescription: {
      en: "The world's first ecosystem blending fintech with social impact. Includes LUCA™ (LGBTQ+ wallet), Inclusivbank™ (seniors/disabled), SAMMY™ (youth literacy), and 1ME™ (migrant identity). Powered by ROP™ (Return on People) metrics.",
      es: "El primer ecosistema del mundo que combina fintech con impacto social. Incluye LUCA™ (billetera LGBTQ+), Inclusivbank™ (mayores/discapacitados), SAMMY™ (alfabetización juvenil) y 1ME™ (identidad migrante). Impulsado por métricas ROP™ (Retorno en Personas).",
      fr: "Le premier écosystème mondial mêlant fintech et impact social. Comprend LUCA™ (portefeuille LGBTQ+), Inclusivbank™ (seniors/handicapés), SAMMY™ (alphabétisation des jeunes) et 1ME™ (identité migrante). Propulsé par les métriques ROP™.",
      de: "Das weltweit erste Ökosystem, das Fintech mit sozialer Wirkung verbindet. Beinhaltet LUCA™ (LGBTQ+-Wallet), Inclusivbank™ (Senioren/Behinderte), SAMMY™ (Jugendbildung) und 1ME™ (Migrantenidentität)."
    }
  },
  {
    id: 'treesla',
    title: 'TREESLA™',
    category: 'Clean Energy / Biotech',
    fundingAsk: '$5M Seed',
    valuation: 'Undisclosed',
    imageUrl: 'https://images.unsplash.com/photo-1501854140884-074bf6b24363?auto=format&fit=crop&w=800&q=80',
    pitchDeckUrl: '',
    tags: ['Biotech', 'Clean Energy', 'Hardware'],
    shortDescription: {
      en: "The world's first non-invasive bio-electrical harvesting platform transforming trees into clean power stations.",
      es: "La primera plataforma de recolección bioeléctrica no invasiva del mundo que transforma árboles en estaciones de energía limpia.",
      fr: "La première plateforme mondiale de récolte bioélectrique non invasive transformant les arbres en centrales électriques propres.",
      de: "Die weltweit erste nicht-invasive bioelektrische Ernteplattform, die Bäume in saubere Kraftwerke verwandelt."
    },
    fullDescription: {
      en: "Turning trees into decentralized power stations using RootSync™ and SapStream™ technology. Validated with MIT Media Lab and Fraunhofer ISE. Addresses a $1.2T renewable market without harming nature.",
      es: "Convierte árboles en estaciones de energía descentralizadas utilizando tecnología RootSync™ y SapStream™. Validado con MIT Media Lab y Fraunhofer ISE. Aborda un mercado renovable de $1.2T sin dañar la naturaleza.",
      fr: "Transforme les arbres en centrales décentralisées via RootSync™ et SapStream™. Validé avec le MIT Media Lab. S'adresse à un marché renouvelable de 1,2 T$ sans nuire à la nature.",
      de: "Verwandelt Bäume mittels RootSync™ und SapStream™ in dezentrale Kraftwerke. Validiert mit dem MIT Media Lab. Adressiert einen 1,2 Bio. $ Markt für erneuerbare Energien."
    }
  },
  {
    id: 'gchs',
    title: 'GCHS™',
    category: 'EdTech / HR Tech',
    fundingAsk: '$10M Seed',
    valuation: 'Undisclosed',
    imageUrl: 'https://images.unsplash.com/photo-1521737604893-d14cc237f11d?auto=format&fit=crop&w=800&q=80',
    pitchDeckUrl: '',
    tags: ['AI', 'Blockchain', 'Social Impact'],
    shortDescription: {
      en: "AI-driven career homologation making professional recognition borderless and verifiable.",
      es: "Homologación profesional impulsada por IA que hace que el reconocimiento profesional sea verificable y sin fronteras.",
      fr: "Homologation de carrière pilotée par l'IA rendant la reconnaissance professionnelle sans frontières et vérifiable.",
      de: "KI-gesteuerte Karrierehomologierung, die berufliche Anerkennung grenzenlos und überprüfbar macht."
    },
    fullDescription: {
      en: "Solving the global talent crisis for 280M migrants. Uses AI and Blockchain to validate credentials instantly. Operates on a 'Half Profit, Half Empowerment' model: every paid verification funds one for a refugee.",
      es: "Resolviendo la crisis de talento global para 280M de migrantes. Utiliza IA y Blockchain para validar credenciales al instante. Opera bajo el modelo 'Mitad Beneficio, Mitad Empoderamiento'.",
      fr: "Résout la crise mondiale des talents pour 280M de migrants. Utilise l'IA et la Blockchain pour valider les diplômes. Modèle 'Half Profit, Half Empowerment' : chaque vérification payée en finance une pour un réfugié.",
      de: "Löst die globale Talentkrise für 280 Mio. Migranten. Nutzt KI und Blockchain zur sofortigen Validierung. Modell 'Halber Gewinn, Halbe Ermächtigung': Jede bezahlte Verifizierung finanziert eine für einen Flüchtling."
    }
  },
  {
    id: 'kronos',
    title: 'KRONOS™ + Evolv™',
    category: 'Web3 / Social Economy',
    fundingAsk: '$5M Pre-Seed',
    valuation: '$25M Cap',
    imageUrl: 'https://images.unsplash.com/photo-1508962914676-134849a727f0?auto=format&fit=crop&w=800&q=80',
    pitchDeckUrl: '',
    tags: ['Crypto', 'Social Media', 'Human Economy'],
    shortDescription: {
      en: "A sovereign human economy where time is currency and kindness is status.",
      es: "Una economía humana soberana donde el tiempo es moneda y la amabilidad es estatus.",
      fr: "Une économie humaine souveraine où le temps est une monnaie et la gentillesse un statut.",
      de: "Eine souveräne menschliche Ökonomie, in der Zeit Währung und Freundlichkeit Status ist."
    },
    fullDescription: {
      en: "Standardizes the value of human time. KRONOS™ token rewards wages and volunteering. Evolv™ is the social platform where 'good vibes' increase your creditworthiness. Includes a debit card and POS system.",
      es: "Estandariza el valor del tiempo humano. El token KRONOS™ recompensa salarios y voluntariado. Evolv™ es la plataforma social donde las 'buenas vibras' aumentan tu solvencia crediticia.",
      fr: "Standardise la valeur du temps humain. Le jeton KRONOS™ récompense les salaires et le bénévolat. Evolv™ est la plateforme sociale où les 'bonnes ondes' augmentent votre solvabilité.",
      de: "Standardisiert den Wert menschlicher Zeit. KRONOS™ Token belohnt Löhne und Ehrenamt. Evolv™ ist die soziale Plattform, auf der 'gute Vibes' Ihre Kreditwürdigkeit erhöhen."
    }
  },
  {
    id: 'solarchain',
    title: 'SOLARCHAIN™',
    category: 'Blockchain / Energy',
    fundingAsk: '$25M Series A',
    valuation: 'Undisclosed',
    imageUrl: 'https://images.unsplash.com/photo-1509391366360-2e959784a276?auto=format&fit=crop&w=800&q=80',
    pitchDeckUrl: '',
    tags: ['Blockchain', 'ESG', 'Solar'],
    shortDescription: {
      en: "The first blockchain defined by the sun, using Proof-of-Solar™ consensus.",
      es: "La primera blockchain definida por el sol, utilizando el consenso Proof-of-Solar™.",
      fr: "La première blockchain définie par le soleil, utilisant le consensus Proof-of-Solar™.",
      de: "Die erste durch die Sonne definierte Blockchain, die den Proof-of-Solar™ Konsens nutzt."
    },
    fullDescription: {
      en: "Revolutionizing blockchain sustainability. Validators must demonstrate real-time solar energy production via IoT oracles. Creates sSUN energy credits and SOLC governance tokens. Targeting $300M ARR.",
      es: "Revolucionando la sostenibilidad blockchain. Los validadores deben demostrar producción de energía solar en tiempo real. Crea créditos de energía sSUN y tokens de gobernanza SOLC.",
      fr: "Révolutionne la durabilité de la blockchain. Les validateurs doivent démontrer une production d'énergie solaire en temps réel. Crée des crédits sSUN et des jetons SOLC.",
      de: "Revolutioniert die Nachhaltigkeit von Blockchains. Validatoren müssen Solarproduktion in Echtzeit nachweisen. Erstellt sSUN-Energiekredite und SOLC-Governance-Token."
    }
  },
  {
    id: 'sammy',
    title: 'SAMMY™',
    category: 'Fintech / EdTech',
    fundingAsk: '$3.5M Series A',
    valuation: 'Undisclosed',
    imageUrl: 'https://images.unsplash.com/photo-1554224155-6726b3ff858f?auto=format&fit=crop&w=800&q=80',
    pitchDeckUrl: '',
    tags: ['Youth', 'Education', 'Fintech'],
    shortDescription: {
      en: "AI-powered, gamified digital wallet designed specifically for children ages 5-18.",
      es: "Billetera digital gamificada impulsada por IA diseñada específicamente para niños de 5 a 18 años.",
      fr: "Portefeuille numérique gamifié alimenté par l'IA conçu spécifiquement pour les enfants de 5 à 18 ans.",
      de: "KI-gesteuerte, spielerische digitale Geldbörse speziell für Kinder von 5-18 Jahren."
    },
    fullDescription: {
      en: "Addressing a $14.2B market crisis in youth financial literacy. Combines real money management with structured education. Bridges seamlessly into the 1ME™ adult financial identity system.",
      es: "Abordando una crisis de mercado de $14.2B en alfabetización financiera juvenil. Combina gestión de dinero real con educación estructurada. Se integra perfectamente con el sistema 1ME™.",
      fr: "Répond à une crise de marché de 14,2 Mrd $ dans la littératie financière des jeunes. Combine gestion d'argent réel et éducation structurée. Transition fluide vers le système 1ME™.",
      de: "Adressiert eine 14,2 Mrd. $ Marktkrise in der Finanzkompetenz von Jugendlichen. Kombiniert echtes Geldmanagement mit strukturierter Bildung. Nahtloser Übergang in das 1ME™ System."
    }
  },
  {
    id: 'luca',
    title: 'LUCA™',
    category: 'Fintech',
    fundingAsk: '$5M Series A',
    valuation: 'Undisclosed',
    imageUrl: 'https://images.unsplash.com/photo-1573164713714-d95e436ab8d6?auto=format&fit=crop&w=800&q=80',
    pitchDeckUrl: '',
    tags: ['LGBTQ+', 'Banking', 'Inclusion'],
    shortDescription: {
      en: "The digital wallet for the global LGBTQ+ community (Love, Unity, Care, Acceptance).",
      es: "La billetera digital para la comunidad LGBTQ+ global (Amor, Unidad, Cuidado, Aceptación).",
      fr: "Le portefeuille numérique pour la communauté LGBTQ+ mondiale (Amour, Unité, Soin, Acceptation).",
      de: "Die digitale Geldbörse für die globale LGBTQ+ Community (Liebe, Einheit, Fürsorge, Akzeptanz)."
    },
    fullDescription: {
      en: "Targeting 400M+ individuals with $3.7T purchasing power. Features 'Chosen Name' display, safety/privacy controls, and a directory of inclusive businesses.",
      es: "Dirigido a más de 400M de personas con un poder adquisitivo de $3.7T. Cuenta con visualización de 'Nombre Elegido', controles de seguridad y directorio de negocios inclusivos.",
      fr: "Ciblant 400M+ individus avec un pouvoir d'achat de 3,7 T$. Affiche le 'Nom Choisi', contrôles de sécurité/confidentialité et annuaire d'entreprises inclusives.",
      de: "Zielt auf 400M+ Individuen mit 3,7 Bio. $ Kaufkraft. Bietet Anzeige des 'Gewählten Namens', Sicherheitskontrollen und ein Verzeichnis inklusiver Unternehmen."
    }
  },
  {
    id: 'ark',
    title: 'The Ark Project',
    category: 'Web3 / Conservation',
    fundingAsk: '£4M Seed',
    valuation: 'Undisclosed',
    imageUrl: 'https://images.unsplash.com/photo-1542273917363-3b1817f69a2d?auto=format&fit=crop&w=800&q=80',
    pitchDeckUrl: '',
    tags: ['NFT', 'Conservation', 'Climate'],
    shortDescription: {
      en: "Turning carbon offsets into digital proof of impact via living NFTs.",
      es: "Convirtiendo compensaciones de carbono en pruebas digitales de impacto mediante NFTs vivos.",
      fr: "Transformer les compensations carbone en preuve numérique d'impact via des NFT vivants.",
      de: "Umwandlung von CO2-Kompensationen in digitale Wirkungsnachweise durch lebende NFTs."
    },
    fullDescription: {
      en: "A living digital sanctuary. NFTs automatically purchase and retire carbon credits. Collections include Forests of the World and Endangered Species. Backed by verified environmental data.",
      es: "Un santuario digital vivo. Los NFTs compran y retiran automáticamente créditos de carbono. Las colecciones incluyen Bosques del Mundo y Especies en Peligro.",
      fr: "Un sanctuaire numérique vivant. Les NFT achètent et retirent automatiquement des crédits carbone. Collections incluant Forêts du Monde et Espèces en Danger.",
      de: "Ein lebendiges digitales Heiligtum. NFTs kaufen und legen automatisch CO2-Gutschriften still. Sammlungen umfassen Wälder der Welt und bedrohte Arten."
    }
  }
];
